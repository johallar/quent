...
import {
  ...
  useTimelineData,
  useSetTimelineHover,
} from '../hooks/useTimelineAtoms';
import { timelineCacheKey } from '../state/timeline';
import { TimelineSkeleton, type TimelineHoverPosition } from '@quent/viz-timeline';
...
import type { TimelineSeries, TimelineMark } from '@quent/viz-timeline';
import { EntityTypeKey } from '@quent/protocol';
import { WHITE, withOpacity, type PaletteTheme } from '@quent/utils';
import { useMinZoomSpanPct } from '@quent/viz-core';
...
const Timeline = lazy(() => import('@quent/viz-timeline').then(mod => ({ default: mod.Timeline })));

export type ResourceTimelineProps = {
  engineId: string;
  queryId: string;
  resourceId: string;
  resourceType: string;
...
export function ResourceTimeline({
  ...
  const renderZoomRange = useZoomRange();
  const minZoomSpanPct = useMinZoomSpanPct(durationSeconds, MIN_ZOOM_WINDOW_S);
  const bulkInitialized = useBulkInitialized();
  const hideTasks = useHideTasks();

  const operatorId = selectedOperatorId;
  const operatorLabel = selectedOperatorLabel;
    ...
    <div className="h-full w-full">
      <Suspense fallback={<TimelineSkeleton />}>
        <Timeline
          ...
          zoomRange={renderZoomRange}
          minZoomSpanPct={minZoomSpanPct}
        />
        {showTooltip && (
