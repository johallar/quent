 ...
 */
export interface ChartGridInsets {
  left: number;
  right: number;
}

const NO_GRID_INSETS: ChartGridInsets = { left: 0, right: 0 };

export function useTimelineWheelNavigation(
  minZoomSpanPct: number,
  gridInsets: ChartGridInsets = NO_GRID_INSETS
) {
    ...
    (instance: EChartsType, wheelTarget: HTMLElement = instance.getDom()) => {
      ...
      const handleWheel = (event: WheelEvent) => {
        ...
        const usableWidth = Math.max(
          1,
          rect.width - gridInsetsRef.current.left - gridInsetsRef.current.right
        );
