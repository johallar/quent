...
/**
 * Single inversion site for the "negative is good" display convention.
 * Raw contract: `delta = baseline - comparison` (positive = comparison faster).
 * Display: invert so "comparison faster than baseline" reads as a negative
 * (cool-colored) number.
 */
export function toDisplayDelta(delta: number): number {
  return delta === 0 || Object.is(delta, -0) ? 0 : -delta;
}

export function toDisplayPercentDelta(percentDelta: number | null): number | null {
