...
describe('buildDiffTimelineData', () => {
  it('uses backend-provided delta series for the diff lane', () => {
    ...
    expect(data.comparisonWithDelta.series.slots?.opacity).toBe(0.5);
    ...
    expect(data.comparisonWithDelta.series['Delta: Baseline higher']?.opacity).toBeUndefined();
    ...
    expect(data.comparisonWithDelta.series['Delta: Comparison higher']?.opacity).toBeUndefined();
