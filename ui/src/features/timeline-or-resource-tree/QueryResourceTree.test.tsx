...
import { applyBulkTimelineResponse, timelineCacheKey } from '@quent/resource-timeline';
import { timelineDataMapAtom } from '@quent/features/testing';
import type { SingleTimelineResponse } from '@quent/protocol';

// ---------------------------------------------------------------------------
// Mock heavy/visual dependencies so tests run without a real browser/canvas
// ---------------------------------------------------------------------------

vi.mock('@quent/features', async importOriginal => {
  const actual = await importOriginal<typeof import('@quent/features')>();
  return {
    ...actual,
    useHighlightedItemIds: () => new Set<string>(),
    useSelectedNodeIds: () => new Set<string>(),
    TreeTable: ({
      columns,
    }: {
...
vi.mock('@quent/resource-timeline', async importOriginal => {
  const actual = await importOriginal<typeof import('@quent/resource-timeline')>();
  return {
    ...actual,
